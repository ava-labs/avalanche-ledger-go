...
func (_m *Ledger) Addresses(numAddresses int) ([]ids.ShortID, error) {
	ret := _m.Called(numAddresses)

	var r0 []ids.ShortID
	if rf, ok := ret.Get(0).(func(int) []ids.ShortID); ok {
		r0 = rf(numAddresses)
	} else {
		if ret.Get(0) != nil {
			r0 = ret.Get(0).([]ids.ShortID)
...
// SignHash provides a mock function with given fields: hash, addressIndexes
func (_m *Ledger) SignHash(hash []byte, addressIndexes []uint32) ([][]byte, error) {
	ret := _m.Called(hash, addressIndexes)

	var r0 [][]byte
	if rf, ok := ret.Get(0).(func([]byte, []uint32) [][]byte); ok {
		r0 = rf(hash, addressIndexes)
	} else {
		if ret.Get(0) != nil {
			r0 = ret.Get(0).([][]byte)
		}
